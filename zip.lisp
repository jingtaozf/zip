...
(defun make-zipfile-writer
    (pathname &key (if-exists :error) (if-does-not-exist :create)
...
(defmacro with-output-to-zipfile
    ((var pathname &key (if-exists :error) (if-does-not-exist :create)) &body body)
...
(defun unzip (pathname target-directory &key (if-exists :error) (if-does-not-exist :create) verbose)
...
(defun zip (pathname source-directory &key (if-exists :error) (if-does-not-exist :create))
  (let ((base (%directory-namestring (merge-pathnames source-directory))))
    (with-output-to-zipfile (zip pathname :if-exists if-exists :if-does-not-exist if-does-not-exist)
      (labels ((recurse (d)
